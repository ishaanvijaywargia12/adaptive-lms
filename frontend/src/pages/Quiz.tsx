      ...
      <div className="max-w-2xl mx-auto pt-10">
        <div className="card text-center py-12 border border-slate-700/50">
          ...
          <div className="text-left space-y-6 mt-8">
            ...
            {result.questionReviews?.map((review: any, idx: number) => (
              <div key={review.questionId} className={`card border ${review.isCorrect ? 'border-emerald-500/50' : 'border-red-500/50'}`}>
                <div className="flex items-start justify-between gap-4 mb-4">
                  <h3 className="text-lg font-medium text-white">
                    <span className="text-slate-400 mr-2">{idx + 1}.</span>
                    {attemptState?.questions?.find((q: any) => q.id === review.questionId)?.questionText || "Question text hidden"}
                  </h3>
                  {review.isCorrect ? (
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex gap-2">
                    ...
                    <span className={review.isCorrect ? 'text-emerald-300' : 'text-red-300'}>
